...
export default memo(function Keyboard() {
	...
	const onSureButtonTap = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
	...
	useEffect(() => {
		const handler = (event: Event) => {
			...
			while (tar.parentElement) {
				const s = JSON.stringify([...Array.prototype.slice.call(tar.classList)].join('/'));
				if (reg.test(s)) {
					tag = true;
					break;
				}
	...
	// render
	return (
		<div className={`lg-keyboard-wrapper ff-DIN-Bold noselect ${visible ? 'visible' : ''}`}>
