...
export default memo(function Input(props: IProps) {
	const { max = Infinity, digits = Infinity, actionText = '确认', align = 'left', clear = true, type = 'default' } = props;
	...
	return (
		<div className="lg-input" onClick={onWrapperTap}>
			...
			<div className={`lg-input__wrapper __${align}`}>
				{letters.length > 0 && (
					<div className={`lg-input__value lg-input__cursor ${focus ? '__cursor' : ''} ${props.valueCls || ''}`} style={{ fontSize: props.fontSize }}>
						{letters.map((ch, i) => (
				...
				{/* 占位符 */}
				{letters.length === 0 && (
					<div className={`lg-input__placeholder lg-input__cursor  ${focus ? '__cursor' : ''} __${align}`} style={{ fontSize: props.fontSize }}>
						{props.placeholder}
					</div>
				)}
			</div>
